...
public class MQFaultStrategy {
    ...
    // 根据currencyLatency本次消息发送延迟，从latencyMax尾部开始向前找到第一个比currencyLatency小的索引，如果没找到，返回0
    // 然后根据这个索引从notAvalibleDuration数组只能够取出对应时间，在这个时间内，broker将不可用
        ...
        if (this.sendLatencyFaultEnable) {
            try {
                ...
                for (int i = 0; i < tpInfo.getMessageQueueList().size(); i++) {
                    ...
                    // 验证每隔MessageQueue是否可用，如果可用，就直接返回当前mq
    ...
    /**
     * 具体算法：从latencyMax数组尾部开始寻找，找到第一个比currentLatency小的下标，
