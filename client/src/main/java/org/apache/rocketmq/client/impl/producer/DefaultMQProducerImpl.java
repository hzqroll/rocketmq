...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    public void start(final boolean startFactory) throws MQClientException {
        switch (this.serviceState) {
            case CREATE_JUST:
                ...
                // 1 检查producerGroup是否符合要求，并改变生产者的instance为进程Id
                this.checkConfig();
                // 如果不是内部标识，切groupName为改变，就用时间戳给一个不重复的
                if (!this.defaultMQProducer.getProducerGroup().equals(MixAll.CLIENT_INNER_PRODUCER_GROUP)) {
                    this.defaultMQProducer.changeInstanceNameToPID();
                }
                // 2 创建MQClientlnstance实例。整个JVM实例中只存在一个MQClientManager实例，维护一个MQClientlnstance缓存表
                // ConcurrentMap<String／＊clientld，MQClientinstanc＞factoryTable=newConcurrentHashMap<Strig，MQClientlnstance＞（），
                // 也就是同一个clientld只会创建一个MQClientinstanc巳。代码清单3而是创建clientld的方法。
                // 一个实例上只能有一个mqClient实例，双重锁判断
                this.mQClientFactory = MQClientManager.getInstance().getOrCreateMQClientInstance(this.defaultMQProducer, rpcHook);

                // 3 向MQClientInstance注册，讲当前生产者加入到MQClientInstance管理中，方便后续的网络请求，心跳检测等。放入到producerTable中进行维护
                ...
                // 启动MQClientInstance如果已经启动，本次不会启动
                if (startFactory) {
     ...
     * @param currentLatency 本次发送的延迟时间
